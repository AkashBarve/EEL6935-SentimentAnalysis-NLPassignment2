...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset, 
    ...
    u1 = outputVectors[target]
    u2 = outputVectors[indices1[1:]]
    cost = - (np.log(sigmoid(np.dot(u1, predicted))) + np.sum( np.log(sigmoid(np.dot(-u2, predicted)))))

    for k in indices1:
        if k == target:
            grad[k] = (sigmoid(np.dot(outputVectors[k], predicted)) - 1.0) * predicted
        else:
            grad[k] = grad[k] + (sigmoid(np.dot(outputVectors[k], predicted))) * predicted


    gradPred = ((sigmoid(np.dot(u1, predicted)) - 1.0) * u1 + np.dot(sigmoid(np.dot(u2, predicted)), u2))
    #raise NotImplementedError
    ### END YOUR CODE
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    for word in contextWords:
        o = tokens[word]
        cost_, gradPred, grad = word2vecCostAndGradient(vc, o, outputVectors, dataset)
        cost = cost + cost_
        gradIn[c] = gradIn[c] + gradPred
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    for cword in contextWords:
        gradIn[tokens[cword]] = gradIn[tokens[cword]] + gradPred
    #raise NotImplementedError
    ### END YOUR CODE
